...
- 2026-02-10 | Caching the CelesTrak JSON catalog could exceed localStorage quota and break live loads | JSON payloads can be significantly larger than TLE and localStorage quotas vary (~5MB typical) | Added cache size cap, swallowed quota errors, and cached smaller TLE text when JSON is too large | When caching external datasets, measure payload sizes and guard writes with size checks + try/catch | 5b51530 | high
...
- 2026-02-10 | `curl -sS 'https://api.github.com/repos/sarveshkapre/AstraView/actions/runs?branch=main&per_page=1'` | latest run `conclusion=success` for commit `30d9036` | pass
- 2026-02-10 | `npm run lint` | `eslint src` | pass
